...
public class SesameRDFVisitor implements RDFVisitor<Object, QueryMetadata>{
    ...
    public Union visit(UnionBlock expr, QueryMetadata md) {
        ...
        Union rv = new Union(tuples.get(0), tuples.get(1));
        for (int i = 2; i < tuples.size(); i++){
            rv = new Union(rv, tuples.get(i));
        }
        return rv;
    }
        ...
        for (Block block : blocks){
            if (block instanceof OptionalBlock){
                if (!tuples.isEmpty()){
                    ...
                    LeftJoin lj = new LeftJoin(tuples.size() == 1 ? tuples.get(0) : join(tuples), right);
    ...
    public ValueExpr visit(Operation<?> expr, QueryMetadata md) {
        ...
        if (op == Ops.AND){
            ...
            return new MathExpr(toValue(expr.getArg(0), md), toValue(ConstantImpl.create(-1), md), MathOp.MULTIPLY);
