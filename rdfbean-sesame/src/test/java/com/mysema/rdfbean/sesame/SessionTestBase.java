...
public class SessionTestBase {
    ...
    @BeforeClass
    public static void setup() throws RepositoryException, RDFParseException, IOException, ClassNotFoundException {
        if (repository == null) {
    ...
    protected static SesameSession createSession(Package... packages) throws RepositoryException, ClassNotFoundException {
    ...
    protected static SesameSession createSession(Locale locale, Package... packages) throws RepositoryException, ClassNotFoundException {
        SesameSession session = new SesameSession(repository.getConnection(), locale, packages);
        session.setIdentityService(MemoryIdentityService.instance());
        return session;
    }

    protected static SesameSession createSession(Class<?>... classes) throws RepositoryException {
        SesameSession session = new SesameSession(repository.getConnection(), classes);
        session.setIdentityService(MemoryIdentityService.instance());
        return session;
    }

    protected static SesameSession createSession(Locale locale, Class<?>... classes) throws RepositoryException {
        SesameSession session = new SesameSession(repository.getConnection(), locale, classes);
        session.setIdentityService(MemoryIdentityService.instance());
        return session;
    }
    
    protected static RepositoryConnection newDefaultConnection() throws RepositoryException {
